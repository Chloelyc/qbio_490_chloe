...
# rna_se <- 
rna_clinical <- read.csv("/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/week7_rna/brca_rna_clinical_data.csv",row.names=1, header=T)
rna_counts <- read.csv("/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/week7_rna/brca_rna_count_data.csv", row.names=1, header=T)
rna_genes <- read.csv("/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/week7_rna/brca_rna_gene_data.csv",row.names=1, header=T)
```
...
# write code here
age_category <- factor(rna_clinical$age_category)
```
...
```{r}
# write code here
gender_category <- factor(rna_clinical$gender)
```
...
sum(is.na(rna_clinical$gender_category))
...
```{r}
na_mask <- ifelse(is.na(rna_clinical$ajcc_pathologic_stage)==T, F, T) # you can use multiple NA masks if you have multiple columns with NA values (or you can do it all in one!)
...
gene_mask <- ifelse(rna_genes$gene_id %in% results@rownames == T, T, F)
results <- data.frame(rna_genes$gene_name[gene_mask], results@rownames, results$log2FoldChange,results$pvalue, results$padj, -log10(results$padj))
...
```{r}
up_reg_results <- sig_results[order(sig_results$log2foldchange, decreasing=T), ]
```
...
up_reg_results <- up_reg_results[!is.na(up_reg_results$log2foldchange),]
```
...
down_reg_results <- down_reg_results[down_reg_results$log2foldchange < -1,]
down_reg_results <- down_reg_results[!is.na(down_reg_results$log2foldchange),]
```
...
```{r}
# write code here
rownames(up_reg_results) <- up_reg_results$gene_name
rownames(down_reg_results) <- down_reg_results$gene_name
```
...
Gene ensembl id:ENSG00000223941.5
Log2FoldChange:2.350917	
p-adj value:9.592276e-11	
More highly expressed in young or old patients: young patients
Google the gene. What is its full name, and what appears to be its function? long
Genecards generally has a pretty good summary:
name: intergenic non-protein coding RNA 1014
function: unipolar depression, plasma clusterin measurement

Gene ensembl id: ENSG00000046774.10
Log2FoldChange: -2.854502	
p-adj value: 1.600169e-13
More highly expressed in young or old patients: old patients
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:
name: Melanoma-associated antigen C2
function: 
1. Proposed to enhance ubiquitin ligase activity of RING-type zinc finger-containing E3 ubiquitin-protein ligases.
2. In vitro enhances ubiquitin ligase activity of TRIM28 and stimulates p53/TP53 ubiquitination in presence of Ubl-conjugating enzyme UBE2H leading to p53/TP53 degradation.
3. Proposed to act through recruitment and/or stabilization of the Ubl-conjugating enzymes (E2) at the E3:substrate complex. 

Write the results table to the week6 folder using write.csv().

```{r}
# write code here
write.csv(results, "/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/week6_maf/deseq_results")
```
...
top-right genes: significantly up-regulated by young patients
bottom-right genes: not significantly up-regulated by young patients
top-left genes: significantly diwn-regulated by young patients
bottom-left genes:not significantly down-regulated by young patients
top-middle genes: significant, but equal expression
bottom-middle genes: not significant and equal expression
...
```{r}
# create plot here
jpeg("/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/week6_maf/volcano.jpg")
 EnhancedVolcano(results,
    lab = rownames(results),
    x = 'log2FoldChange',
    y = 'pvalue',
