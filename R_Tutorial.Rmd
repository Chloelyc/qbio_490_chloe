...
```{r}
# write code here
sum_na <- sum(na_boolean_mask)
sum_na # The number represents the number of NA values in the data set
```
...
```{r}
# write code here
num_row <- nrow(attenu) # num_row shows the total number of rows in attenu
num_row_cleaned <- nrow(attenu_cleaned) # num_row_cleaned shows the number of rows without na values
num_row
num_row_cleaned
```
...
mean_s <- mean(s)
mean_p <- mean(p)
iris$color <- ifelse((s>mean_s)&(p>mean_p), "blue", ifelse((s>mean_s)&(p<mean_p), "deepskyblue", ifelse((s<mean_s)&(p<mean_p), "lightcoral", "darkorchid")))
color_factor <- factor(iris$color)
print(cor.test(s, p))
plot(s, p, col=iris$color)
abline(h=mean_p)
abline(v=mean_s)
...
```{r}
# write code here
pairs(iris[1:4], pch = 21, bg = c("lightcoral", "lightskyblue", "blue")[unclass(iris$Species)])
```
...
```{r}
# write code here
write.csv(attenu_cleaned, "/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/attenu_cleaned.csv")
```
...
```{r}
# write code here
attenu_read_in <- read.csv("/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/attenu_cleaned.csv", header=TRUE)
```
...
```{r}
# write code here
jpeg("/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/histogram.jpg")
hist(iris$Petal.Length)
abline(h  = 1)
dev.off()
```
