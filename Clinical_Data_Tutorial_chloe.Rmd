...
Look up "early onset breast cancer" in Google Scholar and pick a paper to skim.
    1. Link to paper: https://www.sciencedirect.com/science/article/pii/S0960977601903583
    2. What is one thing you learned from this paper: The incidence of breast cancer is increasing in all industrialized countries. Changes in diet and reproductive patterns and altered exposure to endogenous and exogenous substances with hormonal activity have been suggested as contributing to this increase.
...
```{r}
# write code here
clinical$age_category <- ifelse(age<50, "young", "old")
...
```{r}
age_mask <- ifelse(age<50, TRUE, FALSE)
```
...
Call the sum function on age_mask. Remember that TRUEs are treated as 1, and FALSEs are treated as 0. Knowing this, what will your sum represent?
    1. Sum = 309
    Sum represents the number of young patient in the dataframe
...
```{r}
nrow(clinical) == nrow(young_clinical)+nrow(old_clinical)
```
...
clinical$survival_time <- ifelse(is.na(clinical$days_to_death)==FALSE, clinical$days_to_death, clinical$days_to_last_followup)
```
...
The survival probability of old patients is lower than that of young patients, and decreases more rapidly.
...
write.csv(clinical, "/Users/chloelyc/Desktop/QBIO490/qbio_490_chloe/brca_clinical_data.csv", row.names = FALSE)
```
